...
export default function CareersSection() {
  ...
  return (
    <section id="careers" className="py-20 bg-slate-50 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        {/* Header */}
        <div className="text-center max-w-3xl mx-auto mb-12">
          <span className="text-xs uppercase tracking-widest text-blue-700 font-extrabold px-4 py-1.5 rounded-full bg-blue-100 border border-blue-200 inline-block mb-3">
            Join the Innovation Movement
          </span>
          <h2 className="text-4xl sm:text-5xl font-extrabold text-slate-900 tracking-tight">
            Careers &amp; Open Positions at <span className="text-blue-600">VRIF</span>
          </h2>
          <p className="text-slate-600 text-base sm:text-lg mt-4 leading-relaxed font-medium">
            Be part of Karnataka&apos;s premier university incubation network. All recruitments are conducted as per guidelines approved by the <strong className="text-slate-900">Karnataka Innovation and Technology Society (KITS)</strong>.
          </p>
        </div>
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  className="w-full p-6 text-left flex flex-col sm:flex-row sm:items-center justify-between gap-4 cursor-pointer hover:bg-slate-50/80 transition-colors"
                >
                  <div className="space-y-1.5">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {job.stipend && (
                        <span className="text-[11px] font-bold uppercase tracking-wider text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-100 flex items-center gap-1">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        {job.keyHighlights.map((h, i) => (
                          <span
                            key={i}
                            className="text-xs font-semibold text-slate-700 bg-white px-3 py-1 rounded-lg border border-slate-200 shadow-2xs"
                          >
                            ✓ {h}
                          </span>
                        ))}
                      ...
                      <div className="bg-white p-4 rounded-xl border border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs font-medium text-slate-600">
                        <div className="flex items-center gap-2">
                          <MapPin className="w-4 h-4 text-red-500 shrink-0" />
                          <span>Location: <strong className="text-slate-900">{job.location}</strong></span>
                        </div>
